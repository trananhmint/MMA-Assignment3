...
export default function Home() {
    ...
    const renderBrand = ({ item, index }) => {
        return (
            <Button style={styles.button} textColor='orange' buttonColor='white' mode="outlined" onPress={() => changeBrand(item)}>
                {item}
            </Button>
        )
    };
    ...
    const renderWatch = ({ item, index }) => {
        return (
            <CardComponent key={item.id} id={item.id} image={item.image} watchName={item.watchName} price={item.price} index={index} />
        )
    };
